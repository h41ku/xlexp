...
const exportToExcel = (table, fileName) => {
    ...
    const styles = [ { bold: false, align: 'left' } ]
    const getStyleId = style => {
        let i = styles.findIndex(item => item.align === style.align && item.bold === style.bold)
        if (i < 0) {
            i = styles.length
            styles.push(style)
        }
    ...
    table.querySelectorAll('tr').forEach(tr => {
        ...
        tr.querySelectorAll('th, td').forEach(td => {
            if (td.dataset.xlIgnore) {
                return
            }
            const value = td.innerText
            const align = td.dataset.xlAlign ? td.dataset.xlAlign : 'left'
            const bold = td.dataset.xlBold ? true : false
            const styleId = getStyleId({ align, bold })
            let i = strings.indexOf(value)
            if (i < 0) {
                i = strings.push(value) - 1
            }
            row.push(`<c r="${ getColumnNameByIndex(colNo) + rowNo }" s="${ styleId }" t="s"><v>${ i }</v></c>`)
...
<styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main">
  ...
  <cellXfs count="${ styles.length }">
    ${ styles.map(style => `<xf numFmtId="0" fontId="${ style.bold ? 1 : 0 }" fillId="0" borderId="0" xfId="0" applyNumberFormat="1"${ style.bold ? ' applyFont="1"' : '' } applyAlignment="1">
